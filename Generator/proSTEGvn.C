...
void proSTEGvn()
{
  ...
  int mult = 200;//= atoi(getenv("MULT"));

  int tot_num=1000;  //1k events
  ...
  TFile f(Form("vndata_1k_mult%d.root",mult), "RECREATE","ROOT file with histograms & tree");
...
//  tree->Branch("phirg", &b_phirg, "phirg/F");  // RP angle;
  ...
  TF1 *PtDistr  = new TF1("PtDistr","[0]*(exp (-(x/[1]))+[2]*exp (-(x/[3])))", 0.2,6.0);	//Real Data from Pb+Pb 40-50%
  ...
  TF1 *V1vsEta = new TF1("V1vsEta","-exp(-(x+1)^[0])/[1]-x/[2]+exp(-(x-1)^[0])/[1]",-4,4);
 ...
 //	V2vsPt->SetParameters(4.81159,1.80783,3.69272,3.11889,0.931485);	//Real data V~0.05
  ...
  double v1, v2, v3, v4, v5, v6, myphi, mypt, myeta, Psi;
  ...
  for(int i=0; i<tot_num; i++){ 
    ...
    if (i%10 == 0) cout << i << " " << "events processed" << endl;
